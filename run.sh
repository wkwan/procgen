...
export EXPERIMENT_DEFAULT="experiments/dqn.yaml"
...
if [[ " $@ " =~ " --train " ]]; then
  ...
  python3 train.py -vv -f ${EXPERIMENT} --ray-memory ${RAY_MEMORY_LIMIT:-1500000000} --ray-num-cpus ${RAY_CPUS:-2} --ray-object-store-memory ${RAY_STORE_MEMORY:-1000000000}
  STATUS_CODE=$?
fi
